...
def train_net_qyl(param, model, train_data, valid_data, plot=False,device='cuda', save_model_name=''):
    # 初始化参数
    ...
    for epoch in range(epoch_start, epochs):
        ...
        #filename = os.path.join(save_ckpt_dir, 'final_' + save_model_name)
        #torch.save(state, filename)  # pytorch1.6会压缩模型，低版本无法加载
        
        # 保存最优模型
        if iu[1] > best_iou:  # train_loss_per_epoch valid_loss_per_epoch
            state = {'epoch': epoch, 'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict()}
            filename = os.path.join(save_ckpt_dir, save_model_name)

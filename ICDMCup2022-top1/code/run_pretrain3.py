...
def run_pretrain3():
    ...
    logger.logger.info('start run pretrain3.....')
    ...
    #复赛数据加入预训练后的一些实验结果超出了我们的预期，只训练了85轮
    parser.add_argument("--n-epoch", type=int, default=85)
    ...
    #加载复赛测试集节点
    logger.logger.info('loading session2 test file='+args.test_file)
    test_id = [int(x) for x in open(args.test_file).readlines()]
    converted_test_id = []
    for itemid in test_id:
        #注意复赛的item节点用maps2
        converted_test_id.append(hgraph['item'].maps2[itemid])

    test_idx = converted_test_id

    logger.logger.info('session2 item number='+str(len(test_idx)))

    #加载复赛测试集的b和f邻居节点
    session2_test_fnode_labels_path = '../data/session2/session2_test_fnode_labels.pkl'
    session2_test_bnode_labels_path = '../data/session2/session2_test_bnode_labels.pkl'


    with open(session2_test_fnode_labels_path,'rb') as f:
        session2_test_fnode_labels = pkl.load(f)
    with open(session2_test_bnode_labels_path,'rb') as f:
        session2_test_bnode_labels = pkl.load(f)
    
    session2_test_fnode = list(session2_test_fnode_labels.keys())
    session2_test_bnode = list(session2_test_bnode_labels.keys())

    #转换到pyg文件中对应的节点id，注意这里是复赛数据，映射字典用maps2
    session2_test_fnode = [hgraph['f'].maps2[nodeid] for nodeid in session2_test_fnode]
    session2_test_bnode = [hgraph['b'].maps2[nodeid] for nodeid in session2_test_bnode]

    logger.logger.info('session2 test f number='+str(len(session2_test_fnode)))
    logger.logger.info('session2 test b number='+str(len(session2_test_bnode)))


    #合并初赛训练集和复赛测试集节点
    train_idx = train_idx + test_idx
    train_f_idx = list(set(session1_train_fnode + session2_test_fnode))
    train_b_idx = list(set(session1_train_bnode + session2_test_bnode))
    ...
    #这里需要加载之前已经预训练item类型节点500轮又继续添加了b和f节点预训练了300轮的结果
    ...
    for epoch_id in range(epoch):
        ...
        model_name = osp.join('../models', "pretrain_itembf_second_" + str(epoch_id%2) + ".pth")
